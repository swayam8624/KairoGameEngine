...
KAIRO's current portfolio target is **95% across 27 repositories**, but that
target is not self-certified. The earlier blanket 95% claim was retracted after
workspace/build failures proved that frozen scope and status files were being
confused with execution evidence. A revision is accepted only after the exact
`workspace.lock` heads pass the host acceptance campaign and produce a matching
local evidence stamp. See [`docs/V1_95_COMPLETION.md`](docs/V1_95_COMPLETION.md),
[`docs/DEVELOPMENT_MAP.md`](docs/DEVELOPMENT_MAP.md), and
...
Only after that command succeeds, verify that the evidence still matches the
current GameEngine revision and `workspace.lock`:

```bash
bash scripts/verify_portfolio_95.sh
```

The verifier does not read a percentage from YAML as proof. It validates the
exact-head evidence stamp generated by the successful acceptance run.
...
## Sibling Repository Development Workflow

KairoGameEngine uses sibling working trees, not nested submodules. Make and
publish a component change in its owning repository, then update
`workspace.lock` only after that exact head is accepted.
...
Do not run historical `git submodule update` commands for the current
workspace architecture.

## Integration Status

The current exact-head integration state is **unverified until the acceptance
runner succeeds**. Documentation, frozen scope, fetched commits, or a clean
repository do not substitute for configure/build/test/runtime evidence.
...
## Historical Wave Milestones

The former Wave A/B percentages are historical planning milestones only. They
must not be read as current completion evidence. Current acceptance is governed
by `workspace.lock`, `scripts/run_portfolio_acceptance.sh`, and the matching
exact-head evidence stamp.
